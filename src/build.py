...
def altlang(body, lang):
    """Видимая ссылка на другую языковую версию.

    Есть только на русской странице. С английской её убрали намеренно:
    англоязычному посетителю ссылка на русский сайт бесполезна, а доверия
    к продукту не добавляет. Теги hreflang в <head> при этом остаются
    на обеих — они невидимы и нужны поисковику, чтобы он считал страницы
    вариантами одного продукта, а не двумя конкурирующими сайтами."""
    if lang != 'ru':
        new, n = re.subn(r'\s*<p class="alang">.*?</p>', '', body, count=1, flags=re.S)
